...
module.exports = async (message, pages) => {
    if(!pages || !message) throw new TypeError(`You have to supply both message and a pages array!`)
    ...
    pages.forEach(() => {
        ...
        dropdowns.push({
            label: `#${newPos}`,
            description: `Click this to jump to ${newPos} page!`,
            value: `${newPos}`
        })
    })
    ...
    const collector = baseMessage.createMessageComponentCollector({componentType: "SELECT_MENU"})
